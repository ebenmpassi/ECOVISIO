...
def classifier_par_regles(features, seuils=None):
    """
    Décide d'un niveau (vide / a_moitie / pleine) à partir des caractéristiques.
    ...
    Logique :
      ...
      2) Si aucune ne s'applique, on retombe sur la logique par SEUILS
         (densité de contours + texture), elles aussi configurables.
    """
    ...
    if contours < seuils["seuil_contours_bas"]:
        score_contours = 0
    elif contours < seuils["seuil_contours_haut"]:
        score_contours = 1
    else:
        score_contours = 2

    if texture < seuils["seuil_texture_bas"]:
        score_texture = 0
    elif texture < seuils["seuil_texture_haut"]:
        score_texture = 1
    else:
        score_texture = 2

    niveau = round((score_contours + score_texture) / 2)
    niveau = max(0, min(2, niveau))

    label = NIVEAU_VERS_LABEL[niveau]
    explication = (
        f"contours={contours} (score {score_contours}), "
        f"texture={texture} (score {score_texture}) "
