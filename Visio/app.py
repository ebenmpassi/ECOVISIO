...
<div class="wrap">
  ...
  <div class="cartes">
    ...
    <div class="stat orange"><div class="n">{{ rep['a_moitie'] }}</div><div class="l">À moitié</div></div>
  ...
  {% if item %}
    ...
    <form method="post" action="{{ url_for('etiqueter_post', item_id=item['id']) }}">
      <div class="niveaux">
        ...
        <button class="lm" name="label" value="a_moitie">À moitié</button>
...
<div class="wrap">
  ...
  <div class="bloc">
    ...
    <table>
      <tr><th>ID</th><th>Lieu</th><th>Coordonnées</th><th>QR</th></tr>
      {% for p in poubelles %}
      <tr>
        <td>{{ p['id'] }}</td>
        <td>{{ p['nom_lieu'] }}</td>
...
def _generer_id_poubelle():
    ...
    existants = [p["id"] for p in lister_poubelles()]
    n = 1
...
def poubelles():
    return render_template_string(
        PAGE_POUBELLES, poubelles=lister_poubelles(),
